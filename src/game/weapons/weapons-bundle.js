// Weapons Bundle - Combines all bullet types into a single file for backward compatibility

// Include all the bullet class definitions here
// This file acts as a bundle that includes all weapon types
...
// Main Bullet class that creates the appropriate bullet type
class Bullet extends BaseBullet {
    constructor(x, y, angle, owner, type = null) {
        super(x, y, angle, owner, type);
        
        // Return specialized bullet instance based on type
        switch(type) {
            case 'laser':
                return new LaserBullet(x, y, angle, owner);
            case 'rocket':
                return new RocketBullet(x, y, angle, owner);
            case 'explosive':
                return new ExplosiveBullet(x, y, angle, owner);
            case 'piercing':
                return new PiercingBullet(x, y, angle, owner);
            case 'freeze':
                return new FreezeBullet(x, y, angle, owner);
            case 'scatter':
                return new ScatterBullet(x, y, angle, owner);
            default:
                // Return regular bullet (this instance)
                break;
        }
    }
}

// Include all specialized bullet classes
${[
    'LaserBullet',
    'RocketBullet',
    'ExplosiveBullet',
    ...
    // Include the class definitions from the individual files
    // For now, we'll duplicate them here for the bundle
    return '';
}).join('\n')}
